...
$campaign_master = NULL;

//
// Table class for campaign_master
//
class ccampaign_master extends cTable {
	var $campaignID;
	var $title;
	var $title_ar;
	var $campaignSlug;
	...
	function __construct() {
		...
		$this->TableVar = 'campaign_master';
		$this->TableName = 'campaign_master';
		$this->TableType = 'TABLE';

		// Update Table
		$this->UpdateTable = "`campaign_master`";
		$this->DBID = 'DB';
		$this->ExportAll = TRUE;
		...
		// campaignID
		$this->campaignID = new cField('campaign_master', 'campaign_master', 'x_campaignID', 'campaignID', '`campaignID`', '`campaignID`', 3, -1, FALSE, '`campaignID`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'NO');
		...
		// title
		$this->title = new cField('campaign_master', 'campaign_master', 'x_title', 'title', '`title`', '`title`', 200, -1, FALSE, '`title`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->title->Sortable = TRUE; // Allow sort
		$this->fields['title'] = &$this->title;

		// title_ar
		$this->title_ar = new cField('campaign_master', 'campaign_master', 'x_title_ar', 'title_ar', '`title_ar`', '`title_ar`', 200, -1, FALSE, '`title_ar`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->title_ar->Sortable = TRUE; // Allow sort
		$this->fields['title_ar'] = &$this->title_ar;

		// campaignSlug
		$this->campaignSlug = new cField('campaign_master', 'campaign_master', 'x_campaignSlug', 'campaignSlug', '`campaignSlug`', '`campaignSlug`', 200, -1, FALSE, '`campaignSlug`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->campaignSlug->Sortable = TRUE; // Allow sort
		$this->fields['campaignSlug'] = &$this->campaignSlug;

		// dateCreated
		$this->dateCreated = new cField('campaign_master', 'campaign_master', 'x_dateCreated', 'dateCreated', '`dateCreated`', ew_CastDateFieldForLike('`dateCreated`', 7, "DB"), 135, 7, FALSE, '`dateCreated`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		// active
		$this->active = new cField('campaign_master', 'campaign_master', 'x_active', 'active', '`active`', '`active`', 16, -1, FALSE, '`active`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'SELECT');
	...
	function getSqlFrom() { // From
		return ($this->_SqlFrom <> "") ? $this->_SqlFrom : "`campaign_master`";
	}
	...
	function Insert(&$rs) {
		...
		if ($bInsert) {
			...
			$this->campaignID->setDbValue($conn->Insert_ID());
			$rs['campaignID'] = $this->campaignID->DbValue;
		}
		return $bInsert;
	}
	...
	function DeleteSQL(&$rs, $where = "", $curfilter = TRUE) {
		...
		if (is_array($where))
			$where = $this->ArrayToFilter($where);
		if ($rs) {
			if (array_key_exists('campaignID', $rs))
				ew_AddFilter($where, ew_QuotedName('campaignID', $this->DBID) . '=' . ew_QuotedValue($rs['campaignID'], $this->campaignID->FldDataType, $this->DBID));
		}
		$filter = ($curfilter) ? $this->CurrentFilter : "";
		ew_AddFilter($filter, $where);
		if ($filter <> "")
	...
	function KeyFilter() {
		...
		if (!is_numeric($this->campaignID->CurrentValue))
			return "0=1"; // Invalid key
		if (is_null($this->campaignID->CurrentValue))
			return "0=1"; // Invalid key
		else
			$sKeyFilter = str_replace("@campaignID@", ew_AdjustSql($this->campaignID->CurrentValue, $this->DBID), $sKeyFilter); // Replace key value
		return $sKeyFilter;
	}
	...
	function getReturnUrl() {
		...
		if (@$_SESSION[$name] <> "") {
			...
			return "campaign_masterlist.php";
		}
	}
	...
	function GetModalCaption($pageName) {
		...
		elseif ($pageName == "campaign_masteredit.php")
			return $Language->Phrase("Edit");
		elseif ($pageName == "campaign_masteradd.php")
	...
	function GetViewUrl($parm = "") {
		if ($parm <> "")
			$url = $this->KeyUrl("campaign_masterview.php", $this->UrlParm($parm));
		else
			$url = $this->KeyUrl("campaign_masterview.php", $this->UrlParm(EW_TABLE_SHOW_DETAIL . "="));
	...
	function GetAddUrl($parm = "") {
		if ($parm <> "")
			$url = "campaign_masteradd.php?" . $this->UrlParm($parm);
		else
			$url = "campaign_masteradd.php";
		return $this->AddMasterUrl($url);
	}

	// Edit URL
	function GetEditUrl($parm = "") {
		$url = $this->KeyUrl("campaign_masteredit.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
	...
	function GetCopyUrl($parm = "") {
		$url = $this->KeyUrl("campaign_masteradd.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
	...
	function GetDeleteUrl() {
		return $this->KeyUrl("campaign_masterdelete.php", $this->UrlParm());
	}
	...
	function KeyToJson() {
		$json = "";
		$json .= "campaignID:" . ew_VarToJson($this->campaignID->CurrentValue, "number", "'");
		return "{" . $json . "}";
	}
	...
	function KeyUrl($url, $parm = "") {
		...
		if ($parm <> "") $sUrl .= $parm . "&";
		if (!is_null($this->campaignID->CurrentValue)) {
			$sUrl .= "campaignID=" . urlencode($this->campaignID->CurrentValue);
		} else {
	...
	function GetRecordKeys() {
		...
		if (isset($_POST["key_m"])) {
			...
			$isPost = ew_IsPost();
			if ($isPost && isset($_POST["campaignID"]))
				$arKeys[] = $_POST["campaignID"];
			elseif (isset($_GET["campaignID"]))
				$arKeys[] = $_GET["campaignID"];
			else
	...
	function GetKeyFilter() {
		...
		foreach ($arKeys as $key) {
			...
			$this->campaignID->CurrentValue = $key;
