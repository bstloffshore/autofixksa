...
class cclients extends cTable {
	...
	function __construct() {
		...
		// clientID
		$this->clientID = new cField('clients', 'clients', 'x_clientID', 'clientID', '`clientID`', '`clientID`', 3, -1, FALSE, '`clientID`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'NO');
		$this->clientID->Sortable = TRUE; // Allow sort
		$this->clientID->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['clientID'] = &$this->clientID;

		// title
		$this->title = new cField('clients', 'clients', 'x_title', 'title', '`title`', '`title`', 200, -1, FALSE, '`title`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->title->Sortable = TRUE; // Allow sort
		$this->fields['title'] = &$this->title;

		// title_ar
		$this->title_ar = new cField('clients', 'clients', 'x_title_ar', 'title_ar', '`title_ar`', '`title_ar`', 200, -1, FALSE, '`title_ar`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->title_ar->Sortable = TRUE; // Allow sort
		$this->fields['title_ar'] = &$this->title_ar;

		// image
		$this->image = new cField('clients', 'clients', 'x_image', 'image', '`image`', '`image`', 200, -1, TRUE, '`image`', FALSE, FALSE, FALSE, 'IMAGE', 'FILE');
		$this->image->Sortable = TRUE; // Allow sort
		$this->fields['image'] = &$this->image;

		// so
		$this->so = new cField('clients', 'clients', 'x_so', 'so', '`so`', '`so`', 16, -1, FALSE, '`so`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->so->Sortable = TRUE; // Allow sort
		$this->so->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['so'] = &$this->so;

		// active
		$this->active = new cField('clients', 'clients', 'x_active', 'active', '`active`', '`active`', 16, -1, FALSE, '`active`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'SELECT');
	...
	function getSqlFrom() { // From
		return ($this->_SqlFrom <> "") ? $this->_SqlFrom : "`clients`";
	}
	...
	function Insert(&$rs) {
		...
		if ($bInsert) {
			...
			$this->clientID->setDbValue($conn->Insert_ID());
			$rs['clientID'] = $this->clientID->DbValue;
		}
		return $bInsert;
	}
	...
	function DeleteSQL(&$rs, $where = "", $curfilter = TRUE) {
		...
		if ($rs) {
			if (array_key_exists('clientID', $rs))
				ew_AddFilter($where, ew_QuotedName('clientID', $this->DBID) . '=' . ew_QuotedValue($rs['clientID'], $this->clientID->FldDataType, $this->DBID));
		}
		$filter = ($curfilter) ? $this->CurrentFilter : "";
		ew_AddFilter($filter, $where);
		if ($filter <> "")
	...
	function KeyFilter() {
		...
		if (!is_numeric($this->clientID->CurrentValue))
			return "0=1"; // Invalid key
		if (is_null($this->clientID->CurrentValue))
			return "0=1"; // Invalid key
		else
			$sKeyFilter = str_replace("@clientID@", ew_AdjustSql($this->clientID->CurrentValue, $this->DBID), $sKeyFilter); // Replace key value
		return $sKeyFilter;
	}
	...
	function GetModalCaption($pageName) {
		...
		elseif ($pageName == "clientsedit.php")
			return $Language->Phrase("Edit");
		elseif ($pageName == "clientsadd.php")
	...
	function GetViewUrl($parm = "") {
		if ($parm <> "")
			$url = $this->KeyUrl("clientsview.php", $this->UrlParm($parm));
		else
			$url = $this->KeyUrl("clientsview.php", $this->UrlParm(EW_TABLE_SHOW_DETAIL . "="));
	...
	function GetAddUrl($parm = "") {
		if ($parm <> "")
			$url = "clientsadd.php?" . $this->UrlParm($parm);
		else
			$url = "clientsadd.php";
		return $this->AddMasterUrl($url);
	}
	...
	function GetEditUrl($parm = "") {
		$url = $this->KeyUrl("clientsedit.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
	...
	function GetCopyUrl($parm = "") {
		$url = $this->KeyUrl("clientsadd.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
	...
	function KeyToJson() {
		$json = "";
		$json .= "clientID:" . ew_VarToJson($this->clientID->CurrentValue, "number", "'");
		return "{" . $json . "}";
	}
	...
	function KeyUrl($url, $parm = "") {
		...
		if ($parm <> "") $sUrl .= $parm . "&";
		if (!is_null($this->clientID->CurrentValue)) {
			$sUrl .= "clientID=" . urlencode($this->clientID->CurrentValue);
		} else {
	...
	function GetRecordKeys() {
		...
		if (isset($_POST["key_m"])) {
			...
			$isPost = ew_IsPost();
			if ($isPost && isset($_POST["clientID"]))
				$arKeys[] = $_POST["clientID"];
			elseif (isset($_GET["clientID"]))
				$arKeys[] = $_GET["clientID"];
			else
		...
		if (!$Doc->ExportCustom) {
			...
			if ($Doc->Horizontal) { // Horizontal format, write header
				...
				if ($ExportPageType == "view") {
					if ($this->clientID->Exportable) $Doc->ExportCaption($this->clientID);
		...
		while (!$Recordset->EOF && $RecCnt < $StopRec) {
			...
			if (intval($RecCnt) >= intval($StartRec)) {
				...
				if (!$Doc->ExportCustom) {
					...
					if ($ExportPageType == "view") {
						if ($this->clientID->Exportable) $Doc->ExportField($this->clientID);
