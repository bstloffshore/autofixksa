...
class cour_values extends cTable {
	...
	function __construct() {
		...
		// valueID
		$this->valueID = new cField('our_values', 'our_values', 'x_valueID', 'valueID', '`valueID`', '`valueID`', 3, -1, FALSE, '`valueID`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'NO');
		$this->valueID->Sortable = TRUE; // Allow sort
		$this->valueID->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['valueID'] = &$this->valueID;

		// title
		$this->title = new cField('our_values', 'our_values', 'x_title', 'title', '`title`', '`title`', 200, -1, FALSE, '`title`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->title->Sortable = TRUE; // Allow sort
		$this->fields['title'] = &$this->title;

		// title_ar
		$this->title_ar = new cField('our_values', 'our_values', 'x_title_ar', 'title_ar', '`title_ar`', '`title_ar`', 200, -1, FALSE, '`title_ar`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->title_ar->Sortable = TRUE; // Allow sort
		$this->fields['title_ar'] = &$this->title_ar;

		// class
		$this->class = new cField('our_values', 'our_values', 'x_class', 'class', '`class`', '`class`', 200, -1, FALSE, '`class`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->class->Sortable = TRUE; // Allow sort
		$this->fields['class'] = &$this->class;

		// summary
		$this->summary = new cField('our_values', 'our_values', 'x_summary', 'summary', '`summary`', '`summary`', 201, -1, FALSE, '`summary`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXTAREA');
		$this->summary->Sortable = TRUE; // Allow sort
		$this->fields['summary'] = &$this->summary;

		// summary_ar
		$this->summary_ar = new cField('our_values', 'our_values', 'x_summary_ar', 'summary_ar', '`summary_ar`', '`summary_ar`', 201, -1, FALSE, '`summary_ar`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXTAREA');
		$this->summary_ar->Sortable = TRUE; // Allow sort
		$this->fields['summary_ar'] = &$this->summary_ar;

		// so
		$this->so = new cField('our_values', 'our_values', 'x_so', 'so', '`so`', '`so`', 16, -1, FALSE, '`so`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->so->Sortable = TRUE; // Allow sort
		$this->so->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['so'] = &$this->so;

		// active
		$this->active = new cField('our_values', 'our_values', 'x_active', 'active', '`active`', '`active`', 16, -1, FALSE, '`active`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'SELECT');
	...
	function Insert(&$rs) {
		...
		if ($bInsert) {
			...
			$this->valueID->setDbValue($conn->Insert_ID());
			$rs['valueID'] = $this->valueID->DbValue;
		}
		return $bInsert;
	}
	...
	function DeleteSQL(&$rs, $where = "", $curfilter = TRUE) {
		...
		if (is_array($where))
			$where = $this->ArrayToFilter($where);
		if ($rs) {
			if (array_key_exists('valueID', $rs))
				ew_AddFilter($where, ew_QuotedName('valueID', $this->DBID) . '=' . ew_QuotedValue($rs['valueID'], $this->valueID->FldDataType, $this->DBID));
		}
		$filter = ($curfilter) ? $this->CurrentFilter : "";
		ew_AddFilter($filter, $where);
		if ($filter <> "")
	...
	function KeyFilter() {
		...
		if (!is_numeric($this->valueID->CurrentValue))
			return "0=1"; // Invalid key
		if (is_null($this->valueID->CurrentValue))
			return "0=1"; // Invalid key
		else
			$sKeyFilter = str_replace("@valueID@", ew_AdjustSql($this->valueID->CurrentValue, $this->DBID), $sKeyFilter); // Replace key value
		return $sKeyFilter;
	}
	...
	function GetModalCaption($pageName) {
		...
		elseif ($pageName == "our_valuesedit.php")
			return $Language->Phrase("Edit");
		elseif ($pageName == "our_valuesadd.php")
	...
	function KeyToJson() {
		$json = "";
		$json .= "valueID:" . ew_VarToJson($this->valueID->CurrentValue, "number", "'");
		return "{" . $json . "}";
	}
	...
	function KeyUrl($url, $parm = "") {
		...
		if ($parm <> "") $sUrl .= $parm . "&";
		if (!is_null($this->valueID->CurrentValue)) {
			$sUrl .= "valueID=" . urlencode($this->valueID->CurrentValue);
		} else {
	...
	function GetRecordKeys() {
		...
		if (isset($_POST["key_m"])) {
			...
			$isPost = ew_IsPost();
			if ($isPost && isset($_POST["valueID"]))
				$arKeys[] = $_POST["valueID"];
			elseif (isset($_GET["valueID"]))
				$arKeys[] = $_GET["valueID"];
			else
	...
	function LoadListRowValues(&$rs) {
		$this->valueID->setDbValue($rs->fields('valueID'));
		...
		if (!$Doc->ExportCustom) {
			...
			if ($Doc->Horizontal) { // Horizontal format, write header
				...
				if ($ExportPageType == "view") {
					if ($this->valueID->Exportable) $Doc->ExportCaption($this->valueID);
		...
		while (!$Recordset->EOF && $RecCnt < $StopRec) {
			...
			if (intval($RecCnt) >= intval($StartRec)) {
				...
				if (!$Doc->ExportCustom) {
					...
					if ($ExportPageType == "view") {
						if ($this->valueID->Exportable) $Doc->ExportField($this->valueID);
